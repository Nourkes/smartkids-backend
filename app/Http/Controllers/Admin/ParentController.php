    ...
    public function index(Request $request): JsonResponse
    {
        ...
        try {
            $filters = $request->only([
                'statut', 'search', 'profession', 'has_children',
...
public function update(Request $request, ParentModel $parent): JsonResponse
{
    ...
    try {
        ...
        if (!empty($userData)) {
            ...
            \Log::info('User trouvé: ' . ($userModel ? 'Oui' : 'Non'));
            
            if ($userModel) {
                $result = $userModel->update($userData);
                \Log::info('Résultat update users: ' . ($result ? 'Succès' : 'Échec')); 
                
                // Synchroniser les rôles Spatie si nécessaire
                if (isset($userData['role'])) {
                    $userModel->syncRoles([$userData['role']]);
                    \Log::info('Rôles synchronisés: ' . $userData['role']); 
                }
            }
        } else {
        ...
        if ($request->has('profession')) $parentData['profession'] = $request->profession;
        if ($request->filled('telephone')) $parentData['telephone'] = $request->telephone;
        if ($request->has('adresse')) $parentData['adresse'] = $request->adresse;
        if ($request->has('contact_urgence_nom')) $parentData['contact_urgence_nom'] = $request->contact_urgence_nom;
        if ($request->has('contact_urgence_telephone')) $parentData['contact_urgence_telephone'] = $request->contact_urgence_telephone;

        \Log::info('Données parent à mettre à jour', $parentData);

        if (!empty($parentData)) {
            $resultParent = $parent->update($parentData);
            \Log::info('Résultat update parent: ' . ($resultParent ? 'Succès' : 'Échec')); 
        }
